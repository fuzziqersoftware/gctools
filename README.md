...
- Run `make` in the root directory of gctools. Executables will be generated for each tool. These tools all build and run on macOS and Ubuntu, but are untested on other platforms.
...
- __Monopoly CD-ROM__ songs play flawlessly. The original SoundMusicSys implementation drops some notes when playing some of these songs; smssynth doesn't drop notes and the songs sound much more complete.
